...
$project = Join-Path $root 'src\CleanLinks\CleanLinks.csproj'
...
$targetDir = Join-Path $addinsDir 'CleanLinks'
...
$binDir = Join-Path $root "src\CleanLinks\bin\$Configuration"
...
Copy-Item (Join-Path $binDir 'CleanLinks.dll') $targetDir -Force
$pdb = Join-Path $binDir 'CleanLinks.pdb'
if (Test-Path $pdb) { Copy-Item $pdb $targetDir -Force }

Copy-Item (Join-Path $root 'src\CleanLinks\CleanLinks.addin') $addinsDir -Force

Write-Host ''
Write-Host 'Установлено:' -ForegroundColor Green
Write-Host "  манифест : $(Join-Path $addinsDir 'CleanLinks.addin')"
Write-Host "  сборка   : $(Join-Path $targetDir 'CleanLinks.dll')"
Write-Host ''
Write-Host "Запустите Revit $RevitVersion — на ленте появится вкладка «Чистые связи»."
